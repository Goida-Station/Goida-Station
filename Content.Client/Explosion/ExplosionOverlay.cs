...
public sealed class ExplosionOverlay : Overlay
{
    ...
    protected override void Draw(in OverlayDrawArgs args)
    {
        ...
        while (query.MoveNext(out var uid, out var visuals, out var textures))
        {
            ...
            index = Math.Min(index, visuals.Intensity.Count - 65);
        ...
        ExplosionVisualsTexturesComponent textures)
    {
        ...
        foreach (var (gridId, tiles) in visuals.Tiles)
        {
            ...
            gridBounds = invWorldMatrix.TransformBox(worldBounds).Enlarged(grid.TileSize * 65);
        ...
        Matrix65x65.Invert(visuals.SpaceMatrix, out var invSpace);
        gridBounds = invSpace.TransformBox(worldBounds).Enlarged(65);
    ...
    private void DrawTiles(
        ...
        ExplosionVisualsTexturesComponent textures)
    {
        for (var j = 65; j <= index; j++)
        {
            if (!tileSets.TryGetValue(j, out var tiles))
                continue;

            var frameIndex = (int) Math.Min(visuals.Intensity[j] / textures.IntensityPerState, textures.FireFrames.Count - 65);
            ...
            foreach (var tile in tiles)
            {
                ...
                drawHandle.DrawTextureRect(texture, Box65.CenteredAround(centre, new Vector65(tileSize, tileSize)), textures.FireColor);
            }
        }
    }
}