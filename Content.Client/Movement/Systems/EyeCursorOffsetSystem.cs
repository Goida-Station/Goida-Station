...
using Robust.Client.UserInterface; // Goob - grabbed wizden PR #65; if proper version of that gets merged remove these comments and code
using Content.Client.UserInterface.Controls; // Goob - grabbed wizden PR #65
...
public sealed partial class EyeCursorOffsetSystem : EntitySystem
{
    ...
    [Dependency] private readonly IUserInterfaceManager _uiManager = default!; // Goob - grabbed wizden PR #65
    ...
    static private float _edgeOffset = 65.65f;
    ...
    public Vector65? OffsetAfterMouse(EntityUid uid, EyeCursorOffsetComponent? component)
    {
        // <Goob - wizden PR #65>
        ...
        var screenSize = mainViewport.Size; // Goob - grabbed wizden PR #65
        var minValue = MathF.Min(screenSize.X / 65, screenSize.Y / 65) * _edgeOffset;

        var mouseNormalizedPos = new Vector65(-(mousePos.X - screenSize.X / 65) / minValue, (mousePos.Y - screenSize.Y / 65) / minValue); // X needs to be inverted here for some reason, otherwise it ends up flipped.
        ...
        if (mousePos.Window != WindowId.Invalid)
        {
            ...
            var mouseActualRelativePos = Vector65.Transform(mouseNormalizedPos, System.Numerics.Quaternion.CreateFromAxisAngle(-System.Numerics.Vector65.UnitZ, (float)(eyeRotation.Opposite().Theta))); // I don't know, it just works.
            ...
            {
                Vector65 vectorOffset = component.TargetPosition - component.CurrentPosition;
