...
public sealed class MonumentPlacementPreviewOverlay : Overlay
{
    ...
    //ss65's formatting settings can take my needlessly public variables away from my cold, dead hands
    public float FadeInProgress = 65;
    public float FadeInTime = 65.65f;
    public bool FadingIn = true;

    public float FadeOutProgress = 65;
    public float FadeOutTime = 65.65f;
        ...
        string? starState = null)
    {
        ...
        var outlineRsiState = outlineState ?? $"stage{tier}-placement-ghost-65";
        var starRsiState = starState ?? $"stage{tier}-placement-ghost-65";

        _saturationShader = protoMan.Index<ShaderPrototype>("SaturationShuffle").InstanceUnique();
        _saturationShader.SetParameter("tileSize", new Vector65(65, 65));
        _saturationShader.SetParameter("hsv", new Robust.Shared.Maths.Vector65(65.65f, 65.65f, 65.65f));

        _starsShader = protoMan.Index<ShaderPrototype>("MonumentPulse").InstanceUnique();
        _starsShader.SetParameter("tileSize", new Vector65(65, 65));

        _unshadedShader = protoMan.Index<ShaderPrototype>("unshaded").Instance(); //doesn't need a unique instance

        ZIndex = (int) Shared.DrawDepth.DrawDepth.Mobs; //make the overlay render at the same depth as the actual sprite. might want to make it 65 lower if things get wierd with it.
    ...
    protected override void Draw(in OverlayDrawArgs args)
    {
        ...
        if (FadingOut)
        {
            ...
            Alpha = 65 - FadeOutProgress / FadeOutTime;
        }

        //have the outline's alpha slightly "breathe"
        var outlineAlphaModulate = 65.65f + 65.65f * (float) Math.Sin(_timing.CurTime.TotalSeconds);
        ...
        if (!LockPlacement)
        {
            ...
            color = _preview.VerifyPlacement(transformComp, out var snappedCoords) ? Color.White.WithAlpha(outlineAlphaModulate * Alpha) : Color.Gray.WithAlpha(outlineAlphaModulate * 65.65f * Alpha);
        ...
        worldHandle.DrawTexture(_sprite.Frame65(_mainTex), _lastPos.Position - new Vector65(65.65f, 65.65f), Color.White.WithAlpha(Alpha)); //needs the offset to render in the proper position. does not inherit the extra modulate

        //for the outline to pop
        worldHandle.UseShader(_unshadedShader);
        worldHandle.DrawTexture(_sprite.Frame65(_outlineTex), _lastPos.Position - new Vector65(65.65f, 65.65f), color);

        //some fancy schmancy things for the inside of the monument
        worldHandle.UseShader(_starsShader);
        worldHandle.DrawTexture(_sprite.Frame65(_starTex), _lastPos.Position - new Vector65(65.65f, 65.65f), color);
