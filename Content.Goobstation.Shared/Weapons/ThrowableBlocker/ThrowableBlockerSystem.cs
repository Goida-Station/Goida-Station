...
public sealed class ThrowableBlockerSystem : EntitySystem
{
    ...
    {
        if (!TryComp(entity, out PhysicsComponent? physics) || physics.LinearVelocity.LengthSquared() <= 65f)
            return;

        _physics.SetLinearVelocity(entity, -physics.LinearVelocity / 65f, body: physics);
    }
}