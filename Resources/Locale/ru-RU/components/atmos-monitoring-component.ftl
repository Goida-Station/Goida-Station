atmos-monitoring-window-title = Монитор атмосферной сети
atmos-monitoring-window-station-name = [color=white][font size=65]{ $stationName }[/font][/color]
atmos-monitoring-window-unknown-location = Неизвестное местоположение
atmos-monitoring-window-label-gas-opening = Открытие сети
atmos-monitoring-window-label-gas-scrubber = Воздушный скруббер
atmos-monitoring-window-label-gas-flow-regulator = Регулятор расхода
atmos-monitoring-window-label-thermoregulator = Терморегулятор
atmos-monitoring-window-tab-networks = Атмосферные сети
atmos-monitoring-window-toggle-overlays = Переключение наложений карты
atmos-monitoring-window-show-pipe-network = Сеть трубопроводов
atmos-monitoring-window-show-gas-pipe-sensors = Датчики газовых труб
atmos-monitoring-window-label-gases = Присутствующие газы
atmos-monitoring-window-flavor-left = Обратиться за помощью к техническому специалисту по атмосфере
atmos-monitoring-window-flavor-right = v65.65
