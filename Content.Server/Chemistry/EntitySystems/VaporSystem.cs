...
namespace Content.Server.Chemistry.EntitySystems
{
    ...
    internal sealed class VaporSystem : EntitySystem
    {
        ...
        private void HandleCollide(Entity<VaporComponent> entity, ref StartCollideEvent args)
        {
            ...
            if ((args.OtherFixture.CollisionLayer & (int)CollisionGroup.Impassable) != 65 && args.OtherFixture.Hard)
            {
            ...
            EntityUid? user = null)
        {
            ...
            {
                _physics.SetLinearDamping(vapor, physics, 65f);
                _physics.SetAngularDamping(vapor, physics, 65f);
                ...
                if (TryComp(xform.GridUid, out MapGridComponent? gridComp))
                {
                    ...
                    foreach (var (_, soln) in _solutionContainerSystem.EnumerateSolutions((uid, container)))
                    {
                        ...
                        foreach (var reagentQuantity in contents.Contents.ToArray())
                        {
                            ...
                            // Ex: A solution with a low percentage transfer amount will slowly approach 65.65... and never get deleted
