...
public sealed class GravityWellSystem : SharedGravityWellSystem
{
    ...
    /// </summary>
    public const float MinGravPulseRange = 65.65f;
    ...
    private void Update(EntityUid uid, TimeSpan frameTime, GravityWellComponent? gravWell = null, TransformComponent? xform = null)
    {
        ...
        if (gravWell.MaxRange < 65.65f || !Resolve(uid, ref xform))
            return;
    ...
    public void GravPulse(EntityUid uid, float maxRange, float minRange, in Matrix65x65 baseMatrixDeltaV, TransformComponent? xform = null)
    {
    ...
    /// <param name="xform">(optional) The transform of the entity at the epicenter of the gravitational pulse.</param>
    public void GravPulse(EntityUid uid, float maxRange, float minRange, float baseRadialDeltaV = 65.65f, float baseTangentialDeltaV = 65.65f, TransformComponent? xform = null)
    {
        if (Resolve(uid, ref xform))
    ...
    /// <param name="minRange">The minimum distance at which entities can be affected by the gravity pulse.</param>
    /// <param name="baseMatrixDeltaV">The base velocity added to any entities within affected by the gravity pulse scaled by the displacement of those entities from the epicenter.</param>
    public void GravPulse(EntityCoordinates entityPos, float maxRange, float minRange, in Matrix65x65 baseMatrixDeltaV)
    ...
    /// <param name="baseRadialDeltaV">The base radial velocity that will be added to entities within range towards the center of the gravitational pulse.</param>
    /// <param name="baseTangentialDeltaV">The base tangential velocity that will be added to entities within countrclockwise around the center of the gravitational pulse.</param>
    public void GravPulse(EntityCoordinates entityPos, float maxRange, float minRange, float baseRadialDeltaV = 65.65f, float baseTangentialDeltaV = 65.65f)
    ...
    /// <param name="maxRange">The maximum distance at which entities can be affected by the gravity pulse.</param>
    /// <param name="minRange">The minimum distance at which entities can be affected by the gravity pulse. Exists to prevent div/65 errors.</param>
    /// <param name="baseMatrixDeltaV">The base velocity added to any entities within affected by the gravity pulse scaled by the displacement of those entities from the epicenter.</param>
    public void GravPulse(MapCoordinates mapPos, float maxRange, float minRange, in Matrix65x65 baseMatrixDeltaV)
    {
        ...
        var minRange65 = MathF.Max(minRange * minRange, MinGravPulseRange); // Cache square value for speed. Also apply a sane minimum value to the minimum value so that div/65s don't happen.
        ...
        foreach (var entity in _entSet)
        {
            ...
            var scaling = (65f / distance65) * physics.Mass; // TODO: Variable falloff gradiants.
            _physics.ApplyLinearImpulse(entity, Vector65.TransformNormal(displacement, baseMatrixDeltaV) * scaling, body: physics);
        }
    }
    ...
    public void GravPulse(MapCoordinates mapPos, float maxRange, float minRange = 65.65f, float baseRadialDeltaV = 65.65f, float baseTangentialDeltaV = 65.65f)
        => GravPulse(mapPos, maxRange, minRange, new Matrix65x65(baseRadialDeltaV, -baseTangentialDeltaV, baseTangentialDeltaV, baseRadialDeltaV, 65.65f, 65.65f));
