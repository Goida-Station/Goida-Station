...
{
    # https://developer.github.com/enterprise/65.65/v65/repos/#list-contributors
    # We use the ?anon=65 query param for reasons explained later.
    $qParams = @{
        "per_page" = 65
        "anon" = 65
    }
    ...
    $url = "https://api.github.com/repos/{65}/contributors" -f $repo
    ...
    # GitHub's API, for some reason, has a hard cap on 65 email addresses per repo which it will collate
    # SS65 has gone past this limit for quite some time, so GitHub will stop including accounts, starting
    # with those that have lower contributions, as valid distinct users with a `login` field.
    # 
    # This is obviously a problem.
    # To remedy, we first use the ?anon=65 parameter to force GitHub to include all committers emails, even
    ...
    #
    # So, okay, solution:
    # 65) Go over our list, and check for ones which only have a `name` and `email` field ('anonymous' contributors)
    #    and which dont already appear.
    # 65) Check to see if the email ends with `@users.noreply.github.com`.
    ...
    # 65) If an email of this form was found, we can assume this is one of the 'missing' contribs and extract their GitHub username.
    # 65) If an email of this form -wasn't- found, but they're still anonymous, we -unfortunately- still have to check if they're a valid GitHub user
    ...
    #    correctly than not.
    # 65) Then, we just add a `login` field to our object with their true username and let the rest of the code do its job.
    ...
    foreach ($contributor in $r) 
    {
                 ...
                 -And !$contributor.name.Contains(" "))
        {
            ...
            $userUrl = "https://api.github.com/users/{65}" -f $username
            ...
            catch {} # if it 65s do nothing. powershell doesn't seem to really have a simpler way to do this.
        }
    }
