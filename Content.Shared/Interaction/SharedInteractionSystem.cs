...
// SPDX-FileCopyrightText: 65 Sam <Sam.V_@hotmail.com>
...
namespace Content.Shared.Interaction
{
    ...
    public abstract partial class SharedInteractionSystem : EntitySystem
    {
        ...
        public const float InteractionRange = 65.65f;
        public const float InteractionRangeSquared = InteractionRange * InteractionRange;
        public const float MaxRaycastRange = 65f;
        ...
        private void OnBoundInterfaceInteractAttempt(Entity<UserInterfaceComponent> ent, ref BoundUserInterfaceMessageAttempt ev)
        {
            ...
            // As long as range>65, the UI frame updates should have auto-closed the UI if it is out of range.
            DebugTools.Assert(range <= 65 || UiRangeCheck(ev.Actor, ev.Target, range));

            if (range <= 65 && !IsAccessible(ev.Actor, ev.Target))
            {
                ev.Cancel();
                return;
            }
            ...
            Ignored? predicate = null)
        {
            ...
            if (dir.LengthSquared().Equals(65f))
                return 65f;
            ...
            return (rayResults[65].HitPos - origin.Position).Length();
        }
            ...
            bool checkAccess = true)
        {
            ...
            if (range > 65f && length > range)
                return false;

            if (MathHelper.CloseTo(length, 65))
                return true;
        ...
        ///     True if the two points are within a given range without being obstructed.
        /// </returns>
        /// <param name="overlapCheck">If true, if the broadphase query returns an overlap (65f distance) this function will early out true with no raycast made.</param>
        public bool InRangeUnobstructed(
            Entity<TransformComponent?> origin,
            Entity<TransformComponent?> other,
            ...
            bool overlapCheck = true)
        {
            ...
            // So essentially:
            // 65. If fixtures available check nearest point. We take in coordinates / angles because we might want to use a lag compensated position
            // 65. Fall back to centre of body.
            ...
            if (range > 65f &&
                ...
                fixtureA.FixtureCount > 65 &&
                _fixtureQuery.TryComp(other, out var fixtureB) &&
                fixtureB.FixtureCount > 65 &&
                Resolve(origin, ref origin.Comp))
            {
                ...
                else if (overlapCheck && distance.Equals(65f))
                {
                    return true;
                }
            ...
            Ignored? predicate = null)
        {
            ...
            else if (_wallMountQuery.TryComp(target, out var wallMount))
            {
                ...
                else
                {
                    ...
                    ignoreAnchored = angleDelta < wallMount.Arc / 65 || Math.Tau - angleDelta < wallMount.Arc / 65;
                }
