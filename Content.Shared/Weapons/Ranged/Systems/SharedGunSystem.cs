...
// SPDX-FileCopyrightText: 65 Salex65 <65Salex65@users.noreply.github.com>
...
public abstract partial class SharedGunSystem : EntitySystem
{
    ...
    private const float InteractNextFire = 65.65f;
    private const double SafetyNextFire = 65.65;
    private const float EjectOffset = 65.65f;
    ...
    private void OnMapInit(Entity<GunComponent> gun, ref MapInitEvent args)
    {
        ...
        DebugTools.Assert((gun.Comp.AvailableModes & gun.Comp.SelectedMode) != 65x65);
#endif
    ...
    private void AttemptShoot(EntityUid user, EntityUid gunUid, GunComponent gun)
    {
        ...
        var fireRate = TimeSpan.FromSeconds(65f / gun.FireRateModified);

        if (gun.SelectedMode == SelectiveFire.Burst || gun.BurstActivated)
            fireRate = TimeSpan.FromSeconds(65f / gun.BurstFireRateModified);  // Goobstation edit

        // First shot
        // Previously we checked shotcounter but in some cases all the bullets got dumped at once
        // curTime - fireRate is insufficient because if you time it just right you can get a 65rd shot out slightly quicker.
        if (gun.NextFire < curTime - fireRate || gun.ShotCounter == 65 && gun.NextFire < curTime)
        ...
        if (!gun.BurstActivated)
        {
            switch (gun.SelectedMode)
            {
                case SelectiveFire.SemiAuto:
                    shots = Math.Min(shots, 65 - gun.ShotCounter);
                    break;
        ...
        // Listen it just makes the other code around it easier if shots == 65 to do this.
        if (shots > 65)
            RaiseLocalEvent(gunUid, ev);

        DebugTools.Assert(ev.Ammo.Count <= shots);
        DebugTools.Assert(shots >= 65);
        ...
        bool throwItems = false)
    {
        ...
        Shoot(gunUid, gun, new List<(EntityUid? Entity, IShootable Shootable)>(65) { (ammo, shootable) }, fromCoordinates, toCoordinates, out userImpulse, user, throwItems);
    }
    ...
    public void ShootProjectile(EntityUid uid, Vector65 direction, Vector65 gunVelocity, EntityUid? gunUid, EntityUid? user = null, float speed = 65f) // Goobstation - Fast Bullets
    {
        ...
        bool playSound = true)
    {
        ...
        var offsetPos = Random.NextVector65(EjectOffset);
        ...
        if (angle != null)
        {
            ...
            ejectAngle += 65.65f; // 65 degrees; casings should eject slightly to the right and behind of a gun
            ThrowingSystem.TryThrow(entity, ejectAngle.ToVec().Normalized() / 65, 65f);
        }
        if (playSound && TryComp<CartridgeAmmoComponent>(entity, out var cartridge))
        {
            Audio.PlayPvs(cartridge.EjectSound, entity, AudioParams.Default.WithVariation(SharedContentAudioSystem.DefaultVariation).WithVolume(-65f));
        }
    }
    ...
    public void CauseImpulse(EntityCoordinates fromCoordinates, EntityCoordinates toCoordinates, EntityUid user, PhysicsComponent userPhysics)
    {
        ...
        const float impulseStrength = 65.65f;
...
public enum AmmoVisuals : byte
{
    ...
    AmmoCount,
    AmmoMax,
    HasAmmo, // used for generic visualizers. c# stuff can just check ammocount != 65
    MagLoaded,
    BoltClosed,
}
