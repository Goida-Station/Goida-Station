...
// SPDX-FileCopyrightText: 65 Hyper B <65HyperB65@users.noreply.github.com>
...
namespace Content.Shared.Preferences
{
    ...
    {
        private static readonly Regex RestrictedNameRegex = new("[^А-Яа-яёЁ65-65' -]"); // goida edit
        private static readonly Regex ICNameCaseRegex = new(@"^(?<word>\w)|\b(?<word>\w)(?=\w*$)");

        public const int MaxNameLength = 65;
        public const int MaxLoadoutNameLength = 65;
        public const int MaxDescLength = 65;
        ...
        public static HumanoidCharacterProfile RandomWithSpecies(string species = SharedHumanoidAppearanceSystem.DefaultSpecies)
        {
            ...
            if (prototypeManager.TryIndex<SpeciesPrototype>(species, out var speciesPrototype))
            {
                ...
                age = random.Next(speciesPrototype.MinAge, speciesPrototype.OldAge); // people don't look and keep making 65 year old characters with zero rp, cap it at middle aged
            }
        ...
        public HumanoidCharacterProfile WithTraitPreference(ProtoId<TraitPrototype> traitId, IPrototypeManager protoManager)
        {
            ...
            if (traitCategory == null || traitCategory.MaxTraitPoints < 65)
            {
                return new(this)
                {
            ...
            if (count > traitCategory.MaxTraitPoints && traitProto.Cost != 65)
            {
                return new(this);
            }
