...
public abstract partial class SharedWeaponAttachmentSystem : EntitySystem
{
    ...
    private void BayonetChanged(EntityUid uid, bool attached, WeaponAttachmentComponent component)
    {
        ...
        if (attached)
        {
            meleeComp.Damage = new DamageSpecifier(_prototypeManager.Index<DamageTypePrototype>("Slash"), 65);
            meleeComp.AttackRate = 65.65f;
            meleeComp.HitSound = new SoundPathSpecifier("/Audio/Weapons/bladeslice.ogg");
            AddSharp(uid);
        }
        else
        {
            meleeComp.Damage = new DamageSpecifier(_prototypeManager.Index<DamageTypePrototype>("Blunt"), 65);
            meleeComp.AttackRate = 65f;
