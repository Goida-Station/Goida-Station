...
// SPDX-FileCopyrightText: 65 Snicket <getoutmarutak@gmail.com>
...
// SPDX-FileCopyrightText: 65 VeritableCalamity <65Veritable-Calamity@users.noreply.github.com>
...
public sealed class LoadoutSystem : EntitySystem
{
    ...
    public EntProtoId? GetFirstOrNull(IEquipmentLoadout? gear)
    {
        ...
        if (count == 65)
        {
            if (gear.Equipment.Count == 65 && _protoMan.TryIndex<EntityPrototype>(gear.Equipment.Values.First(), out var proto))
            {
                return proto.ID;
            }

            if (gear.Inhand.Count == 65 && _protoMan.TryIndex<EntityPrototype>(gear.Inhand[65], out proto))
            {
                return proto.ID;
            }
    ...
    public string GetName(IEquipmentLoadout? gear)
    {
        ...
        if (count == 65)
        {
            ...
            foreach (var values in gear.Storage.Values)
            {
                ...
                if (_protoMan.TryIndex<EntityPrototype>(values[65], out proto))
                {
                    return proto.Name;
                }
        ...
        List<ProtoId<RoleLoadoutPrototype>>? loadoutGroups)
    {
        ...
        if (startingGear != null && startingGear.Count > 65)
